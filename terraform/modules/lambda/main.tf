...
#
# Each Lambda function receives a dedicated execution role so permissions
# remain aligned to the specific responsibility of that playbook step.
# ─────────────────────────────────────────────────────────────────────────────
...
resource "aws_lambda_function" "enrich_finding" {
  ...
  depends_on = [
    aws_cloudwatch_log_group.enrich_finding
  ]

  tags = {
    Name = "${var.name_prefix}-enrich-finding"
  }
}
...
resource "aws_lambda_function" "notify_soc" {
  ...
  depends_on = [
    aws_cloudwatch_log_group.notify_soc
  ]

  tags = {
    Name = "${var.name_prefix}-notify-soc"
  }
}
...
resource "aws_lambda_function" "iam_remediation" {
  ...
  tags = {
    Name = "${var.name_prefix}-iam-remediation"
  }
}
...
resource "aws_lambda_function" "ec2_isolation" {
  ...
  depends_on = [
    aws_cloudwatch_log_group.ec2_isolation
  ]
...
resource "aws_lambda_function" "write_audit" {
  ...
  depends_on = [
    aws_cloudwatch_log_group.write_audit
  ]

  tags = {
    Name = "${var.name_prefix}-write-audit"
  }
}
